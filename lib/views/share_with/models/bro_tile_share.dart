...
class _BroTileShareState extends State<BroTileShare> {
  ...
  bool isAnimatedWebP(Uint8List bytes) {
    ...
    print("image.numFrames: ${image.numFrames}");
  ...
  shareWithBroup(BuildContext context, Broup broupShare) async {
    ...
    print("we are currently sharing: ${sharedFiles}");
    if (sharedFiles.length > 1) {
      ...
      print("shareFilePath: $shareFilePath");

      if (mediaType == SharedMediaType.TEXT) {
        print("text share $SharedMediaType");
        dataType = null;
      } else if (mediaType == SharedMediaType.IMAGE) {
        print("image share $SharedMediaType");
        ...
        print("video share $SharedMediaType");
        final fileExtension = shareFilePath.split('.').last.toLowerCase();
        ...
        print("file share $SharedMediaType");
        final fileExtension = shareFilePath.split('.').last.toLowerCase();
        ...
        print("other share $SharedMediaType");
        dataType = DataType.other.value;
      } else if (mediaType == SharedMediaType.URL) {
        print("url share $SharedMediaType");
        fromUrl = true;
        dataType = null;
      }
      if (dataType == null) {
        ...
        print("sending file with datatype: $dataType");
